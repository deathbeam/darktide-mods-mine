-- File: RingHud/scripts/mods/RingHud/team/constants.lua
...
C.START_X, C.START_Y           = 80, 650 -- was 80, 650
...
C.THROWABLE_ICON_SIZE          = C.TILE_SIZE / 10 -- was /16
...
-- Default multiplier ~= 220px / 200px = 1.10 so the visual width fits neatly.
...
-- Default multiplier ~= 140px / 200px = 0.70 to match visual height.
...
-- These are scale-aware and recomputed from the cached settings on each call.
...
function C.recompute_edge_marker_size()
    ...
    -- Update clamping base (used by pack step helpers and elsewhere)
    C.TILE_SIZE          = 240 * s
...
-- Optional alias on mod.* (keeps cross-file callers flexible)
...
-- Total ledge-hang window used for the countdown fraction in the VM.
-- Kept explicit here so builds without PlayerCharacterConstants still behave.
C.LEDGE_TOTAL_WINDOW                   = PlayerCharacterConstants.time_until_fall_down_from_hang_ledge or 2.5

-- Stimm/pocketables context tunables
C.STIMM_PICKUP_LATCH_SEC               = 10
C.STIMM_CORRUPTION_HP_THRESHOLD        = 0.75

-- NEW: durations for wield-driven latches
C.LOCAL_WIELD_LATCH_SEC                = 10 -- heal-tool reassurance latch (local only)
C.WIELD_POCKETABLE_LATCH_SEC           = 10 -- any stimm/crate → team full-opacity latch
...
-- Lowest-priority stimm glyph (aiming a stimm at a teammate)
C.STATUS_ICON_MATERIALS.stimm          = C.STATUS_ICON_MATERIALS.stimm or "content/ui/materials/hud/icons/party_stimm"
...
"content/ui/materials/mission_board/circumstances/special_waves_03" -- listg chevron, glowing
