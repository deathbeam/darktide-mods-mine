-- File: RingHud/scripts/mods/RingHud/systems/edge_stack.lua
...
--   * All math below operates in **fragment space** (the UI's 1920×1080 virtual grid).
...
--   * `margins` are normalized { left, right, up, down } in 0..1 of screen size (still fine — we multiply by fragment width/height).
...
-- Fragment-space helpers (virtual 1920×1080 grid)
...
-- Per-edge baseline clamp bias, reset each frame (fragment units)
...
function Edge.reset_if_needed()
    ...
    if cur ~= mod._edge_stack_last_frame then
        ...
        -- reset per-edge biases (fragment space)
...
-- Clamp a proposed push so widget.offset + local art-bounds + push stay inside screen edges (fragment space).
...
-- Compute & apply push based on which screen edge the marker is near (fragment space).
...
function Edge.update_push(widget, marker, style, step_x, step_y, margins)
    ...
    local L, R, Tm, B = _fragment_edges(margins)
    local off         = widget.offset or { 0, 0, 0 }
    local x, y        = off[1], off[2]
    local EPS         = mod.EDGE_MARGIN_EPS or 10 -- fragment-space epsilon
    ...
    if edge then
        ...
        if axis == "y" then
            ...
            if x then
                ...
                local gate  = tonumber(mod.EDGE_COLLIDE_FRAG)
                    or tonumber(mod.EDGE_COLLIDE_PX) -- legacy override accepted
                    or _default_gate_frag()
                ...
                if edge == "top" then
                    if refs.top_x == nil then
                        ...
                        if math.abs(x - refs.top_x) <= gate then
                            collides = true
                    ...
                    if refs.bottom_x == nil then
                        ...
                        if math.abs(x - refs.bottom_x) <= gate then
                            collides = true
...
function Edge.screen_edges_px(margins)
    -- Kept name for compatibility with callers, but returns **fragment-space** edges now.
    return _fragment_edges(margins)
end
