...
function _calculate_stagger_strength(
	...
	armor_type,
	lerp_values,
	optional_stagger_strength_multiplier,
	target_stat_buffs,
	attacker_stat_buffs,
	attacker_unit_or_nil,
	target_unit_or_nil
)
	...
	local charged_power_level =
		PowerLevel.scale_by_charge_level(power_level, charge_level, damage_profile.charge_level_scaler)
	local stagger_power_level, scaled_power_level = DamageProfile.power_distribution_from_power_level(
		charged_power_level,
		"impact",
		damage_profile,
		target_settings,
	...
	local stagger_buff_modifier = _calculate_stagger_buffs(
		attack_type,
		target_stat_buffs,
		attacker_stat_buffs,
		armor_type,
...
function _stagger_reduction(
	damage_profile,
	breed,
	is_finesse_hit,
	stagger_reduction_override_or_nil,
	attack_type,
	attacker_stat_buffs,
	hit_weakspot,
	is_burning
	...
	local stagger_reduction = stagger_reduction_override_or_nil
		or attack_type == attack_types.ranged and breed.stagger_reduction_ranged
		or breed.stagger_reduction
	...
	local stagger_reduction_buff_modifier =
		_calculate_stagger_reduction_buffs(attacker_stat_buffs, hit_weakspot, is_burning)
...
function _get_stagger_type(
	stagger_strength,
	damage_profile,
	breed,
	stagger_resistance,
	hit_shield,
	optional_mutator_stagger_overrides,
	hit_weakspot
)
	...
	local threshold_overrides = optional_mutator_stagger_overrides
		and optional_mutator_stagger_overrides.stagger_thresholds
		and optional_mutator_stagger_overrides.stagger_thresholds[breed.name]
	...
	for i = 1, num_stagger_types do
		...
		if stagger_threshold >= 0 then
			local stagger_resistance_modifier = hit_weakspot and damage_profile.weakspot_stagger_resistance_modifier
				or damage_profile.stagger_resistance_modifier
				or 1
...
function _calculate_stagger_buffs(
	attack_type,
	target_stat_buffs,
	attacker_stat_buffs,
	armor_type,
	...
	local attacker_melee_impact_modifier = attack_type == attack_types.melee
			and attacker_stat_buffs.melee_impact_modifier
		or 1
	local attacker_push_impact_modifier = attack_type == attack_types.push and attacker_stat_buffs.push_impact_modifier
		or 1
	local attacker_ranged_impact_modifier = attack_type == attack_types.ranged
			and attacker_stat_buffs.ranged_impact_modifier
		or 1
	local attacker_explosion_impact_modifier = attack_type == attack_types.explosion
			and attacker_stat_buffs.explosion_impact_modifier
		or 1
	local attacker_shout_impact_modifier = attack_type == attack_types.shout
			and attacker_stat_buffs.shout_impact_modifier
		or 1
	local attacker_melee_weakspot_impact_modifier = attack_type == attack_types.melee
			and hit_weakspot
			and attacker_stat_buffs.melee_weakspot_impact_modifier
		or 1
	local attacker_super_armor_crit_impact_modifier = armor_type == armor_types.super_armor
			and is_critical_strike
			and attacker_stat_buffs.super_armor_crit_impact_modifier
		or 1
	local num_modifiers = 9
	local modifier = attacker_impact_modifier
		+ target_impact_modifier
		+ attacker_melee_impact_modifier
		+ attacker_push_impact_modifier
		+ attacker_ranged_impact_modifier
		+ attacker_explosion_impact_modifier
		+ attacker_shout_impact_modifier
		+ attacker_melee_weakspot_impact_modifier
		+ attacker_super_armor_crit_impact_modifier
		- (num_modifiers - 1)
...
StaggerCalculation.calculate = function(
	...
	if target_settings.power_level_multiplier then
		local power_level_lerp_value =
			DamageProfile.lerp_value_from_path(lerp_values, "targets", 1, "power_level_multiplier")
		local power_level_multiplier =
	...
	local stagger_strength, stagger_power_level = _calculate_stagger_strength(
		damage_profile,
		target_settings,
		power_level,
		charge_level,
	...
	local stagger_reduction = _stagger_reduction(
		damage_profile,
		breed,
		is_finesse_hit,
		stagger_reduction_override_or_nil,
	...
	local armor_damage_modifier = DamageProfile.armor_damage_modifier(
		"impact",
		damage_profile,
		target_settings,
		lerp_values,
	...
	local stagger_type, stagger_threshold = _get_stagger_type(
		sum_stagger_strength,
		damage_profile,
		breed,
		stagger_resistance,
		hit_shield,
	...
	if stagger_type then
		mod:error(
			"\nEnemy: "
				.. tostring(Localize(breed.display_name))
				.. "\nStagger Type: "
				.. tostring(stagger_type)
				.. "\nYour Stagger Strength: "
				.. tostring(sum_stagger_strength)
				.. "\nEnemy Stagger Threshold: "
				.. tostring(stagger_threshold)
		)
	else
		mod:error(
			"\nEnemy: "
