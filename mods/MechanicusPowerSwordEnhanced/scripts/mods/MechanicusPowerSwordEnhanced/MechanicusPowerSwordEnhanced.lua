            ...
            if mod_settings.toggle_mode then
                ...
                if weapon_extra_pressed then
                    if special_activated then
                        special_activated = false
                    elseif slot_primary_component and slot_primary_component.num_special_charges >= 1 then
...
local function on_action_start(id, action_name)
    if id == "weapon_action" then
        if mod_settings.toggle_mode and mod_settings.deactivate_on_wield and weapon_action_component and MECHANICUS_POWERSOWRD_WEAPON_NAMES[weapon_action_component.template_name] and action_name == "action_wield" then
            special_activated = false
        end
    end
end
...
local function on_action_finish(id)
    if id == "weapon_action" then
        if mod_settings.toggle_mode and mod_settings.deactivate_on_depleted and slot_primary_component and slot_primary_component.num_special_charges < 1 then
            special_activated = false
        end
    end
end
...
mod:hook_safe(CLASS.ActionHandler, "start_action",
    function(self, id, action_objects, action_name, action_params, action_settings, used_input, t, transition_type,
        ...
        on_action_start(id, action_name)
    end)
