...
local function _start_expedition_submit_input_pulse(t)
	...
	expedition_autosolve_input_pulse_until = math.max(expedition_autosolve_input_pulse_until or 0, t + 0.14)
...
_is_networked_live_minigame = function(minigame)
	return minigame ~= nil and not _practice_session_blocks_live_autosolve() and _is_networked_live_session() and minigame == (_active_live_minigame and _active_live_minigame() or nil)
end
...
mod._expedition_grid_for_cell = function(minigame, target_x, target_y)
	...
	return mod._expedition_board_grid_at(symbols, target_x, target_y)
end
...
mod._debug_expedition_autosolve = function(minigame, reason, extra)
	...
	local direct_target = mod._normalize_expedition_target_position(raw_target)
	...
	local exact_match = mod._expedition_grids_equal(cursor_grid, target_grid)
...
mod._expedition_target_cell = function(minigame)
	...
	if target_grid and type(raw_target) == "table" then
...
mod._expedition_autosolve_move_vector = function(minigame)
	...
	if delta_x == 0 and delta_y == 0 then
		...
		local target_grid = mod._expedition_target_grid(minigame)
		local cursor_grid = mod._expedition_cursor_grid(minigame)
		local exact_target_match = mod._expedition_grids_equal(cursor_grid, target_grid)

		if _is_networked_live_minigame(minigame)
			and exact_target_match
			and state_name == MinigameSettings.game_states.gameplay
			and now >= (mod._expedition_autosolve_submit_retry_at or 0)
		then
			local simulated = _start_expedition_submit_input_pulse(now)

			mod._expedition_autosolve_submit_retry_at = now + 0.2
			expedition_autosolve_cooldown = _expedition_autosolve_cooldown_seconds()

			mod._debug_expedition_autosolve(minigame, "submit", {
				chosen = target,
				move = Vector3.zero(),
				note = simulated and "move_vector_submit" or "move_vector_submit_missing_input",
			})
		end
...
local function _handle_expedition_autosolve_input(action_name, result)
	...
	if action_name == "move" then
		if type(result) == "userdata" then
			local result_x = result.x or 0
			local result_y = result.y or 0
			local result_z = result.z or 0
			local result_magnitude = math.sqrt(result_x * result_x + result_y * result_y + result_z * result_z)

			-- Live expeditions can report small non-zero move vectors even when the player
			-- is not intentionally steering. Treat that as idle so autosolve can drive.
			if result_magnitude >= 0.6 then
				return result
			end
		end
	...
	if _is_expedition_on_target(minigame, gameplay_time) then
		...
		if not mod._expedition_grids_equal(cursor_grid, target_grid) then
...
_active_live_minigame = function()
	...
	local field_minigame = current_state and current_state._minigame or nil
	local field_type = _minigame_type_hint(field_minigame)

	if current_state and current_state.minigame then
		method_minigame = current_state:minigame() or nil
...
local function _looks_like_expedition_minigame(minigame)
	return _looks_like_expedition_grid_minigame(minigame)
		or (_is_expedition_minigame_type(_minigame_type_hint(minigame)) and _looks_like_decode_style_minigame(minigame))
end
...
_active_expedition_autosolve_minigame = function()
	...
	if _looks_like_expedition_minigame(live_minigame) and (live_minigame_type == nil or _is_expedition_minigame_type(live_minigame_type)) then
		return live_minigame
	end
...
local function _minigame_display_mode(minigame_type)
	if mod._is_expedition_map_minigame_type(minigame_type) or _is_expedition_minigame_type(minigame_type) then
...
mod._persistent_expedition_map_minigame = function()
	...
	if not ok_selected or selected_level == nil then
		mod._debug_event("exp_map_minimap", "pending not_ready")
	end
...
_ensure_live_expedition_map_overlay = function(live_minigame)
	...
	if resolved_live_minigame and _looks_like_expedition_minigame(resolved_live_minigame) and not mod._is_expedition_map_minigame_type(live_minigame_type) then
		mod._debug_event("exp_map_overlay", string.format("skip live_minigame=%s", tostring(live_minigame_type or "expedition")))
		return
	end

	local map_minigame = mod._resolve_expedition_map_minigame(false) or resolved_live_minigame
...
mod:hook_require("scripts/extension_systems/character_state_machine/character_states/player_character_state_minigame", function(PlayerCharacterStateMinigame)
	...
	mod:hook_safe(PlayerCharacterStateMinigame, "on_enter", function(self)
		...
		local game_mode_manager = Managers.state and Managers.state.game_mode
		local game_mode_name = game_mode_manager and game_mode_manager:game_mode_name() or nil
		local minigame = self and self._minigame or nil
		local minigame_type = _minigame_type_hint(minigame)
		local unit_data_extension = self and self._unit and ScriptUnit.has_extension(self._unit, "unit_data_system") or nil
		local inventory_component = self and self._inventory_component or (unit_data_extension and unit_data_extension:read_component("inventory")) or nil
	...
	mod:hook_safe(PlayerCharacterStateMinigame, "on_exit", function(self)
		local minigame_character_state = self._minigame_character_state_component
		local interface_synced = minigame_character_state and (minigame_character_state.interface_level_unit_id ~= NetworkConstants.invalid_level_unit_id or minigame_character_state.interface_game_object_id ~= NetworkConstants.invalid_game_object_id) or false
...
mod:hook_require("scripts/extension_systems/scanner_display/scanner_display_extension", function(ScannerDisplayExtension)
	mod:hook(ScannerDisplayExtension, "_open_view", function(func, self, ui_manager, device_owner_unit, interface_unit)
		...
		local current_live_minigame = explicit_expedition_minigame or _try_minigame_extension_minigame(minigame_extension)
		...
		if state_live_minigame and _is_expedition_minigame_type(state_live_minigame_type) and _looks_like_expedition_minigame(state_live_minigame) then
			current_live_minigame = state_live_minigame
			live_minigame = state_live_minigame
			minigame_type = state_live_minigame_type
		...
		if current_live_minigame then
			local current_live_minigame_type = _minigame_type_hint(current_live_minigame) or minigame_type
...
mod:hook_require("scripts/ui/views/scanner_display_view/minigame_expedition_map_view", function(MinigameExpeditionMapView)
	...
	local function _refresh_expedition_map_navigation_context(view)
		...
		if navigation_changed or minigame_changed then
			...
			mod._debug_event("exp_map_minimap", string.format("target reset nav_changed=%s minigame_changed=%s", tostring(navigation_changed), tostring(minigame_changed)))
		end
	...
	local function _prune_expedition_target_widgets(view, widgets, active_keys)
		if not widgets then
			return
		end
	...
	mod:hook_safe(MinigameExpeditionMapView, "update", function(self)
	...
	mod:hook(MinigameExpeditionMapView, "_update_target_widgets", function(func, self, widgets_by_name, navigation_handler)
		...
		local registered_exits = navigation_handler and navigation_handler.get_registered_exits and navigation_handler:get_registered_exits() or nil
		local registered_extractions = navigation_handler and navigation_handler.get_registered_extractions and navigation_handler:get_registered_extractions() or nil

		_prune_expedition_target_widgets(self, self._exit_widgets, _active_expedition_target_keys(registered_exits))
		_prune_expedition_target_widgets(self, self._extraction_widgets, _active_expedition_target_keys(registered_extractions))
...
_update_live_autosolve_input = function(minigame, t)
	...
	if _is_expedition_autosolve_enabled() and _looks_like_expedition_minigame(minigame) then
		...
		if exact_target_match and _is_expedition_on_target(minigame, t) and t >= (mod._expedition_autosolve_submit_retry_at or 0) then
			...
			mod._debug_expedition_autosolve(minigame, "submit", {
				note = is_networked_live and (simulated and "input_service_press" or "input_service_missing") or "live_direct_submit",
			})
		end
	end
...
mod:hook_require("scripts/extension_systems/character_state_machine/character_states/player_character_state_minigame", function(PlayerCharacterStateMinigame)
	mod:hook(PlayerCharacterStateMinigame, "_update_input", function(func, self, t, fixed_frame, input_extension)
		...
		if (_practice_session_blocks_live_autosolve() and not allow_item_mode_practice_autosolve) or (not _is_drill_autosolve_enabled() and not _is_expedition_autosolve_enabled()) then
			return func(self, t, fixed_frame, input_extension)
		end

		local minigame = (self and self._minigame) or (allow_item_mode_practice_autosolve and session.minigame) or nil

		if not _looks_like_drill_minigame(minigame) and not _looks_like_expedition_minigame(minigame) then
			mod._sync_drill_autosolve_minigame(nil)
			return func(self, t, fixed_frame, input_extension)
		end

		if _looks_like_expedition_minigame(minigame) and _is_networked_live_minigame(minigame) and not allow_item_mode_practice_autosolve then
			return func(self, t, fixed_frame, input_extension)
		end
		...
		local expedition_branch = _looks_like_expedition_minigame(minigame) and _is_expedition_autosolve_enabled()
		...
		block_weapon_actions = minigame:blocks_weapon_actions() or practice_expedition_primary_active
...
function mod.on_setting_changed(setting_id)
	...
	elseif setting_id == "enable_expedition_minigame" and not _is_expedition_autosolve_enabled() then
		_reset_expedition_autosolve()
	elseif setting_id == "expedition_map_display_mode" or setting_id == "enable_expedition_map_minigame" or setting_id == "expedition_map_always_minimap" or setting_id == "expedition_map_zoom_scale" then
...
function mod.update(dt)
	...
	if mod._forced_expedition_map_overlay_active then
		...
		if not (ui_manager and ui_manager:view_active(OVERLAY_VIEW_NAME))
			or not scanner_searching_active
			or not expedition_map_minigame
			or _minigame_display_mode(EXPEDITION_MAP_MINIGAME_TYPE) ~= "overlay" then
			mod._forced_expedition_map_overlay_active = nil
