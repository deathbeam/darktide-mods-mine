...
local function _expedition_map_uses_minimap_layout(minigame_type, force_minimap_layout)
	return force_minimap_layout == true and minigame_type == EXPEDITION_MAP_MINIGAME_TYPE
end
...
local function _expedition_map_view_ready(view)
	if not view or view._minigame_type ~= EXPEDITION_MAP_MINIGAME_TYPE then
		return true
	end
	...
	if not minigame then
		local minigame_extension = minigame_view and minigame_view._minigame_extension
		minigame = minigame_extension and minigame_extension.minigame and (minigame_extension:minigame(EXPEDITION_MAP_MINIGAME_TYPE) or minigame_extension:minigame()) or nil
	end

	if mod._is_expedition_map_minigame_ready then
		return mod._is_expedition_map_minigame_ready(minigame)
	end
...
function AuspexOverlayView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	...
	local ok, draw_error = pcall(AuspexOverlayView.super._draw_widgets, self, dt, t, input_service, ui_renderer, render_settings)
